...
- [ ] **CardItem 卡牌选择（打通 OK/Cancel/Discard 全链路）**：参考旧版 `src/uibackup/roomscene.cpp` 的 `useSelectedCard()`(2479)/`doOkButton()`(3083)/`doCancelButton()`(3100)/`doDiscardButton()`(3193)/`updateStatus()`(2743) 与 `src/uibackup/dashboard.cpp` 的 `getSelected`/`pendingCard`/`startPending`/`unselectAll`/`enableCards`。整条链路依赖较多，拆为以下子任务（按依赖顺序）：
  ...
  - [x] **F. 目标选择**：Photo 单击进入目标选择（status=`Playing`/`Responding` 且当前选中卡需目标，按 `card.targetFixed(Self)` 判断）；收集 `selected_targets`；Photo 视觉高亮可选目标（按 `card.targetFilter`，桥接 `Q_INVOKABLE enabledTargetsForCard(card, selectedTargetNames)` 返回可选 player objectName 列表，传已选目标给 targetFilter）；选满后 OK 启用。
...
- **askForCard 与 askForUseCard 共用 `S_COMMAND_RESPONSE_CARD`**，区别在 `Card::HandlingMethod`（请求第 3 字段）。`Client::askForCardOrUseCard`（client.cpp:1141）按 method 映射 status（basic 都是 `Responding`，高位字节区分）：
  ...
  - `MethodDiscard` → `RespondingForDiscard`（0x0201）——弃牌响应（如反馈要弃牌），无目标。
  - `MethodNone`/其他 → `RespondingNonTrigger`（0x0301）——展示/拼点类，无目标。
  - **回传统一走 `Client::onPlayerResponseCard(card, targets)`**（client.cpp:773）：发 `JsonArray() << card->toString() << targetNames`。`askForCard` 服务器读 `[0]` 忽略 targets，`askForUseCard` 服务器读整体。故 `submitCardResponse()` 对四种变体通用（targets 对纯响应类为空）。
- **`updateAuxSkill()`（status_changed 时调）按 pattern + exact status 配 `m_responseSkill`**：pattern 非 `@@`（卡牌 pattern 如 "slash"/"jink"/".|.|.|hand"）时设 `m_responseSkill->setPattern(p)` + `setRequest(method)`（按 exact status：RespondingUse→MethodUse / ForDiscard→MethodDiscard / NonTrigger→MethodNone / 纯 Responding→MethodResponse，对标旧 `updateStatus` `uibackup/roomscene.cpp:2864-2871`）+ `m_currentViewAsSkill = m_responseSkill`。pattern 为 `@@skillname` 时 `m_currentViewAsSkill` 留空（QML 显式驱动，G 任务）。`Playing` 不设 request（无 pattern 匹配，用 `isAvailable`）。
- **`updateHandCardEnabled()`（手牌可选过滤）**：`Playing` → `card->isAvailable(Self)`（出牌阶段自由出牌，只查 `isCardLimited(card, handling_method)`）。`Responding*` 且 `m_currentViewAsSkill != nullptr`（卡牌 pattern）→ **走技能路径 `m_currentViewAsSkill->viewFilter(selectedCards, card)`**（`ViewAsSkill` 多态虚接口，对标旧 `startPending`→`updatePending`），而非 `ResponseSkill::matchPattern`（matchPattern 等价于单参 `viewFilter(card)`，但跳过 `OneCardViewAsSkill::viewFilter(selected, card)` 的 `selected.isEmpty()` 单选守卫与 `hasFlag("using")` 守卫）。已选中的牌保持 enabled（允许点击取消选中）；其余牌由 viewFilter 判定。`Responding*` 且 `@@skill` pattern（`m_currentViewAsSkill` 空）→ 回退 `isAvailable` 作过渡（G 任务用 `skillViewFilter` 逐牌判 viewFilter 替代）。`setRequest` 设的 method 经 viewFilter→`matchPattern` 的 `isCardLimited(card, request)` 生效。
- **`selectCard()` 选牌后重新求值手牌 enablement**：捕获 `m_selectedCardIds` 后立即调 `updateHandCardEnabled()`，使 viewFilter 的 `selected.isEmpty()` 守卫随选牌状态生效（OneCard 技能选满一张后其余 disabled；取消选中后重新 enabled）。对标旧 `startPending`→`updatePending` 在选牌变化时重算。
- **目标选择仅 `Playing` / `RespondingUse`**（使用牌需玩家选目标）：`selectCard()` 选非 targetFixed 牌后，纯 `Responding` / `RespondingForDiscard` / `RespondingNonTrigger` 跳过目标选择（响应/弃牌/展示无玩家目标，对标旧 `enableTargets` `uibackup/roomscene.cpp:1415-1418` no-target-selection 分支、`useSelectedCard:2490-2493` 对纯 Responding `Q_ASSERT(selected_targets.isEmpty())`）。targetFixed 牌任何状态都跳过。
- **`recomputeOkReadiness()` OK 就绪判定**：no-target-selection（targetFixed 牌、纯 `Responding`/`ForDiscard`/`NonTrigger`、`AskForShowOrPindian`）→ 选牌即 OK 就绪；`Playing`/`RespondingUse` 非 targetFixed 牌 → `isCardTargetsFeasible(card, selectedTargets)`（`Card::targetsFeasible`，需至少一个目标）。`AskForSkillInvoke` → true；`Discarding`/`Exchanging` → `!m_selectedCardIds.isEmpty()`；`AskForGeneralTaken` → `activeBox.canAccept`。
...
- **Dashboard 按钮**：`anchors.bottom: cardArea.top` + `bottomMargin` + `horizontalCenter` 浮在手牌区上方；268×133，font.pixelSize 50。**enabled 按 `Client::status`**（条件映射仍同旧版）：Discard=`Playing`；Cancel=`ExecDialog`/`AskForSkillInvoke` 或（`Responding`系列/`Discarding`/`Exchanging` 且 `discardActionRefusable`）；OK=`AskForSkillInvoke` 或响应/弃牌状态的选牌就绪（见"选牌与目标选择"小节 `recomputeOkReadiness`）或活动 box `canAccept`（`AskForGeneralTaken`）。`Client::Status` 经 `Q_ENUM` + uncreatable 注册，QML 用 `Client.Playing` 等枚举名比较；`discardActionRefusable` 经 `Q_PROPERTY` 暴露。**按钮 enabled 由 C++ `RoomScene` 驱动**（非 QML Dashboard）：`RoomScene` 持 `Q_PROPERTY okEnabled/cancelEnabled/discardEnabled`（READ + NOTIFY），Dashboard 三按钮 `enabled` 绑 `roomScene.okEnabled` 等；`updateDashboardStatus()`（C++，`status_changed` 时调）设 cancel/discard + 清选牌状态 + `updateHandCardEnabled()` + `recomputeOkReadiness()`；`recomputeOkReadiness()` 由选牌/选目标/`activeBox.canAccept` 变化触发（选牌与目标选择细节见"选牌与目标选择（Responding 系列）"小节）。Dashboard QML 只剩 visual（按钮转发 + 手牌创建/lay），逻辑全在 C++。
...
### 2026-07-26：F 目标选择修复（enabledTargetsForCard 传已选目标 + Photo parent 误用）
- `src/qmlui/roomscene.h/cpp`：`enabledTargetsForCard`/`enabledTargets` 加 `const QStringList &selectedTargetNames` 参数——实现把 selectedTargetNames 经 `client->getPlayer(name)` 转 `QList<const Player*> selected`，传给 `card->targetFilter(selected, player, Self, maxVotes)`（替代原 `emptySel`）。`targetFilter` 语义是"在已选目标基础上查 to_select 可行性"，不传已选目标会导致依赖已选状态的牌判定错误。
- `qml/RoomScene.qml`：`onCardSelected` 调 `enabledTargetsForCard(card, selectedTargets)` 传已选目标；加 `console.log` 排查（打印 cardId/card/targetFixed/status basic/enabledTargetNames，待确认后删）；`_syncPhotoTargets` 加 log（打印每个 photo 的 objectName/targetable/parent.targetSelectionActive，待删）。
- `qml/Photo.qml`：目标高亮 Rectangle 与目标点击 MouseArea 的 `parent.targetSelectionActive`/`parent.toggleTarget` 误用 `parent`（= Photo 本身，无此 property/method）→ 改为 `photo.parent.targetSelectionActive`/`photo.parent.toggleTarget`（Photo 的 parent = RoomScene root，有 targetSelectionActive property 和 toggleTarget 方法）。根因：QML 中 Rectangle/MouseArea 内的 `parent` 指其视觉 parent（Photo），非 Photo 的 parent（RoomScene root）。

### 2026-07-26：OK 就绪用 targetsFeasible + updateStatus 职责分离
- **设计**：`updateStatus` 只做 status 相关的按钮/手牌 enablement（cancelEnabled/discardEnabled/手牌 isAvailable/unselect），不再设 okEnabled。OK 就绪单独由 `updateOkReadiness()` 处理，用 `Card::targetsFeasible` 判定（对标旧版 `ok_button->setEnabled(card->targetsFeasible(selected_targets, Self))`，旧版在 `enableTargets`/`updateSelectedTargets` 调，`updateStatus` 不负责 OK 就绪）。
- `src/qmlui/roomscene.h/cpp`：加 `Q_INVOKABLE isCardTargetsFeasible(card, selectedTargetNames)`（包装 `Card::targetsFeasible`，selectedTargetNames 经 `client->getPlayer` 转 `QList<const Player*>`）。
- `qml/RoomScene.qml`：加 `property var selectedCard: null`，`onCardSelected` 设/清（reset 时 null，选牌时 card）。`updateOkReadiness` 读 `rs.selectedCard` + `rs.selectedTargets` 判定。
- `qml/Dashboard.qml`：`updateStatus` 移除所有 `okEnabled` 设置，末尾调 `updateOkReadiness`。新增 `updateOkReadiness()`：按 status 分支——AskForSkillInvoke→true；AskForGeneralTaken→box.canAccept；Playing/Responding/AskForShowOrPindian→`selectedCard.targetFixed(Self) || isCardTargetsFeasible(selectedCard, selectedTargets)`；Discarding/Exchanging→`selectedCardIds().length > 0`；其余 false。触发点：`updateStatus`（status 变）、`addHandCard` clicked connect（选牌）、Connections `selectedTargetsChanged`（选目标）、Connections `canAcceptChanged`（box 变）。

### 2026-07-26：选牌逻辑全迁 C++（CppPhoto + CppRoomScene 扩展）
- **设计**：选牌状态与逻辑全迁 C++，QML 只留 visual（绑定 C++ property + 转发点击）。对标旧版 uibackup（RoomScene 持有 selected_targets + enableTargets/updateSelectedTargets/useCard 逻辑，Dashboard/Photo 是视图）。消除 QML 侧逻辑 bug（parent 误用、okEnabled 不更新等）。
- **新建 `src/qmlui/cpphoto.h/cpp`**：`CppPhoto`（继承 QQuickItem）Q_PROPERTY `targetable`/`targetSelected`/`playerName`（C++ 驱动目标高亮/选中态，playerName 供 syncPhotoTargets 匹配）。注册为 QML 类型 `CppPhoto`。
- **`src/qmlui/roomscene.h/cpp`** 扩展：Q_PROPERTY `okEnabled`/`cancelEnabled`/`discardEnabled`（READ+NOTIFY）。Q_INVOKABLE `selectCard(selectedCardIds)`（传完整选中列表支持多选；内部 reset→getCard→targetFixed→status 检查→enabledTargetsForCard→syncPhotoTargets→recomputeOkReadiness）、`toggleTarget(playerName)`、`updateDashboardStatus()`（status 变时设 cancel/discard + 清选牌状态）、`registerPhoto`/`unregisterPhoto`。内部 `recomputeOkReadiness()`（AskForSkillInvoke→true；Playing/Responding/AskForShowOrPindian→`targetFixed||isCardTargetsFeasible`；Discarding/Exchanging→`!m_selectedCardIds.isEmpty()`；AskForGeneralTaken 不设由 QML 绑 activeBox.canAccept）、`syncPhotoTargets()`（遍历 m_photos 按 playerName 匹配设 targetable/targetSelected）。`submitCardResponse(cardIds)`/`respondCard(card)` 去 targetNames 参数（C++ 内部用 m_selectedTargets）。
- **`qml/Photo.qml`**：root 改 `CppPhoto`，移除 `targetable`/`targetSelected` QML property（用 C++ Q_PROPERTY），加 `playerName: player.objectName`。目标高亮/点击绑 `photo.targetable && !photo.selfPhoto`（去掉 parent.targetSelectionActive），点击调 `photo.parent.toggleTarget`。
- **`qml/RoomScene.qml`**：移除选牌状态 property + 函数（onCardSelected/toggleTarget/_syncPhotoTargets）+ onSelectedTargetsChanged。Photo 创建后调 `roomScene.registerPhoto(photo)`。
...
### 2026-08-01：补全 askForCard 处理（Responding 系列 pattern 过滤 + 目标选择区分）
- 旧版代码中 `askForCardOrUseCard` 桥接只针对 `askForUseCard`（使用牌）做了处理，`askForCard`（响应牌，如被杀要闪、要无懈）的 pattern 过滤与目标选择未落地。`askForCard` 与 `askForUseCard` 共用 `S_COMMAND_RESPONSE_CARD`，区别在 `Card::HandlingMethod`（`MethodResponse`/`MethodUse`/`MethodDiscard`/`MethodNone`），客户端映射为 `Responding`/`RespondingUse`/`RespondingForDiscard`/`RespondingNonTrigger`（basic 都是 `Responding`）。本次因 askForCard 选牌/OK 链路不通，补全四处：
  - `updateAuxSkill()`：`Responding` 分支按 exact status 调 `m_responseSkill->setRequest(method)`（对标旧 `updateStatus` `uibackup/roomscene.cpp:2864-2871`），使 `ResponseSkill::matchPattern` 的 `isCardLimited(card, request)` 用对方法（原来恒为默认 `MethodResponse`）。
  - `updateHandCardEnabled()`：`Responding*` 状态从 `card->isAvailable(Self)`（只查 `isCardLimited`，不查 pattern）改为 `m_responseSkill->matchPattern(Self, card)`（查 pattern + `isCardLimited(method)`），使被要求出"闪"时只有闪可选（原来所有未受限牌都可选）。`Playing` 仍用 `isAvailable`；`@@skill` pattern（G 任务）`m_currentViewAsSkill` 非 response skill，回退 `isAvailable` 作过渡。
  - `selectCard()`：目标选择仅限 `Playing`/`RespondingUse`（使用牌需选目标）；纯 `Responding`/`RespondingForDiscard`/`RespondingNonTrigger` 是响应/弃牌/展示，无玩家目标，跳过目标选择（对标旧 `enableTargets` `uibackup/roomscene.cpp:1415-1418` no-target-selection 分支；旧 `useSelectedCard` `:2490-2493` 对纯 `Responding` `Q_ASSERT(selected_targets.isEmpty())`）。
  - `recomputeOkReadiness()`：纯 `Responding`/`RespondingForDiscard`/`RespondingNonTrigger`（及 targetFixed 牌、AskForShowOrPindian）选牌即 OK 就绪（无目标可行性检查）；仅 `Playing`/`RespondingUse` 非 targetFixed 牌走 `isCardTargetsFeasible`。

### 2026-08-01：PromptBox 响应结束后隐藏
- `Client::setStatus`（client.cpp:909）转入 `NotActive` 时（`old_status != status`）清空 `prompt_doc` + `emit promptTextChanged(QString())`，使 QML `PromptBox`（`visible` 绑 `promptText !== ""`）在交互结束后隐藏。对标旧 `updateStatus` NotActive 分支（`uibackup/roomscene.cpp:2800` `prompt_doc->clear()` + `prompt_box->disappear()`）。`processServerRequest` 开头先 `setStatus(NotActive)` 再分发命令，故新请求设新 prompt 前旧 prompt 已清（同 C++ 调用栈内两次 emit，QML 绑定合并无闪烁）。仅 prompt 非空时 emit（NOTIFY 仅值变化时发）。

### 2026-08-01：ResponseSkill card filtering 走 viewFilter 技能路径
- `updateHandCardEnabled()`（roomscene.cpp:945）Responding 分支从 `m_responseSkill->matchPattern(Self, card)` 改为 `m_currentViewAsSkill->viewFilter(selectedCards, card)`（`ViewAsSkill` 多态虚接口，对标旧 `startPending`→`updatePending`）。`matchPattern` 等价于单参 `viewFilter(card)`，但跳过了 `OneCardViewAsSkill::viewFilter(selected, card)`（skill.cpp:343）的 `selected.isEmpty()` 单选守卫与 `hasFlag("using")` 守卫——导致 Responding 下 `toggleCardSelected` 纯 toggle 可选多张、`viewAs(多张)` 对 OneCard 返回 nullptr 误取消。改后：已选中牌保持 enabled（允许取消选中），其余由 viewFilter 判定（选满一张后 disabled）。
- `selectCard()`（roomscene.cpp:743）捕获 `m_selectedCardIds` 后立即调 `updateHandCardEnabled()` 重新求值，使 viewFilter 的 `selected.isEmpty()` 守卫随选牌状态生效（对标旧 `updatePending` 在选牌变化时重算）。
- 确认：ResponseSkill 的 submit 本就走 `m_currentViewAsSkill->viewAs(cards)`（技能路径）；name-based `skillViewFilter`/`skillViewAs`/`submitSkillCard` 用 `Sanguosha->getSkill(name)` 查不到未注册的 ResponseSkill（Client aux-skill），且当前无 QML 调用方（G 任务），故 ResponseSkill 走 direct-pointer 路径（与旧 `startPending` 一致）而非 name-based。
...
- `qml/PlayerCardBox.qml`：基于 GraphicsBox，直接从 `player` 对象查 `handcardIds()/getShownHandcards()/equipIds()/getJudgingAreaID()`，QML 侧组装 areas + disabled（`disabledIds.indexOf(id)>=0 || (method===Card.MethodDiscard && id!==-1 && !roomScene.Self.canDiscard(player,id))`）+ footnote（shown→`Sanguosha.translate("shown_card")`、判定牌→`Sanguosha.translate(getEngineCard(id).objectName)`）。**部分公开手牌时隐藏手牌只画一个牌背**（`!handcardVisible && player!==Self` 时 shownHandcards 作真牌 + 一个 `cardId=-1` back，hidden=`player.handcard - shownIds.length`）。标题 `qsTr("%1: please choose %2's card").arg(Sanguosha.translate(reason)).arg(getPlayerName(player.objectName))`。CardItem `onClicked` → `cardChosen` signal → `ClientInstance.onPlayerChooseCard` + destroy。无取消（必须选）。请求参数（player/flags/reason/handcardVisible/method/disabledIds/enableEmptyCard）由 `RoomScene.qml` `onNotifyCardsGot` 经 `createObject` 传入。
- 注册：`qml/PlayerCardBox.qml` 加入 `.pro` lupdate_only 列表；运行时从 `qml/` 目录磁盘加载，同目录隐式类型解析。

### 2026-08-01：Dashboard 公开手牌 footnote
- 旧版 `Dashboard::updateShown()`（`uibackup/dashboard.cpp:1533-1580`）对 self 的公开手牌（`isShownHandcard(id)`）加 `shown_card` footnote 并连 `showncards_changed` 信号；QML 版此前完全没处理。本次补全：
  - `CardContainer` 新增 `Q_INVOKABLE setCardFootnote(int, QString)`（`setProperty("footnoteText", ...)`，与 `setCardEnabled` 同模式）。
  - `RoomScene` 新增 `Q_INVOKABLE updateShownFootnotes()`：遍历 `m_cardContainer->cardItemIds()`，`Self->isShownHandcard(id)` → footnote=`Sanguosha->translate("shown_card")`，否则清空。`registerCardContainer` 时连 `Self->showncards_changed` → `updateShownFootnotes`（对标旧连法）；`updateDashboardStatus` 末尾调用；`RoomScene.qml` 手牌 move add/remove（`onNotifyMoveCardsGot`/`Lost` 的 PlaceHand 分支）后也调用（覆盖 shown 牌重新进手牌）。
