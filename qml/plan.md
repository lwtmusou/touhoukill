...
- **`updateHandCardEnabled()`（手牌可选过滤）**：`Playing` → `card->isAvailable(Self)`（出牌阶段自由出牌，只查 `isCardLimited(card, handling_method)`）。`Responding*` 且 `m_currentViewAsSkill == m_responseSkill`（卡牌 pattern）→ `m_responseSkill->matchPattern(Self, card)`（查 `pattern->match` + `isCardLimited(card, request)`），使被要求出"闪"时只有闪可选（`isAvailable` 不查 pattern，会放行所有未受限牌，故不可用）。`Responding*` 且 `@@skill` pattern（`m_currentViewAsSkill` 空）→ 回退 `isAvailable` 作过渡（G 任务用 `skillViewFilter` 逐牌判 viewFilter 替代）。
