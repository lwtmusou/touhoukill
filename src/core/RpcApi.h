...
public:
  RequestResult()
    : m_result(static_cast<T>(0)), m_state(RequestStatus::Ivalid)
  {
  }

  RequestResult(T t, RequestStatus state = RequestStatus::Normal)
    : m_result(t), m_state(state)
  {
  ...
  RequestStatus state() const {
    return m_state;
  }

  void setState(RequestStatus state) {
    m_state = state;
  }
...
public:
   ...
   */
  RequestResult<int> askForCardChosen(Player *player, Player *target, const QString &flags, const QString &reason, bool handcard_visible = false, QSanguosha::HandlingMethod method = QSanguosha::MethodNone, const QList<int> &disabled_ids = {}, int time_limit = 0);
