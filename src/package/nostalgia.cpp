...
bool XihuaCard::xihua(ServerPlayer *yuji) const{
    Room *room = yuji->getRoom();
    QList<ServerPlayer *> players = room->getOtherPlayers(yuji);
    //选择展示者
	ServerPlayer *splayer = room->askForPlayerChosen(yuji, players, "xihua", "@xihua", false, true);
	int card_id = room->askForCardChosen(splayer, yuji, "h", "xihua");
	const Card *card =Sanguosha->getCard(card_id);
	//QSet<ServerPlayer *> questioned;
   ...
   // foreach (int card_id, getSubcards())
	...
	bool real;
        if (user_string == "peach+analeptic")
            real = card->isKindOf("BasicCard");
        else if (user_string == "slash")
            real = card->isKindOf("BasicCard");
        else if (user_string == "normal_slash")
            real = card->isKindOf("BasicCard");
		else if (user_string == "jink")
            real = card->isKindOf("BasicCard");
        else
            real = card->isKindOf("TrickCard");
    if (card->getNumber()>10 ||real) {
        success = true;
        //foreach (ServerPlayer *player, players)
        //    room->setEmotion(player, ".");
		yuji->tag["xihua_show"] = QVariant(card_id);
        CardMoveReason reason(CardMoveReason::S_REASON_USE, yuji->objectName(), QString(), "xihua");
        CardsMoveStruct move(used_cards, yuji, NULL, Player::PlaceUnknown, Player::PlaceTable, reason);
        moves.append(move);
        room->moveCardsAtomic(moves, true);
    } else {//戏画失败后的惩罚
        room->moveCardTo(this, yuji, NULL, Player::DiscardPile,
                             CardMoveReason(CardMoveReason::S_REASON_PUT, yuji->objectName(), QString(), "xihua"), true);
		QString pattern=user_string.split("+").first();
		QString pattern1;
		if (pattern=="slash" || pattern=="fire_slash" ||pattern=="thunder_slash")
			pattern1="Slash";
		if (pattern=="jink")
			pattern1="Jink";
		if (pattern=="peach")
			pattern1="Peach";
		if (pattern=="analeptic")
			pattern1="Analeptic";
		if (pattern=="amazing_grace")
			pattern1="AmazingGrace";
		if (pattern=="god_salvation")
			pattern1="GodSalvation";
		if (pattern=="savage_assault")
			pattern1="SavageAssault";
		if (pattern=="archery_attack")
			pattern1="ArcheryAttack";
		if (pattern=="duel")
			pattern1="Duel";
		if (pattern=="ex_nihilo")
			pattern1="ExNihilo";
		if (pattern=="snatch")
			pattern1="Snatch";
		if (pattern=="dismantlement")
			pattern1="Dismantlement";
		if (pattern=="collateral")
			pattern1="Collateral";
		if (pattern=="iron_chain")
			pattern1="IronChain";
		if (pattern=="fire_attack")
			pattern1="FireAttack";
		if (pattern=="nullification")
			pattern1="Nullification";
		room->setPlayerCardLimitation(yuji, "use,response", pattern1, true);
		room->setPlayerMark(yuji, "xihua_limit",yuji->getMark("xihua_limit")+1);
		//QString num=<string>x+y
		QString tagname= "xihua_limit"+yuji->getMark("xihua_limit");//.tostring();
		yuji->tag[tagname] = QVariant(pattern1);
		//room->setTag(tagname, pattern1);
	}
    yuji->tag.remove("NosGuhuoSaveSelf");
    yuji->tag.remove("NosGuhuoSlash");
    return success;
...
bool XihuaCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const{
    ...
    CardStar card = Self->tag.value("xihua").value<CardStar>();
    return card && card->targetFilter(targets, to_select, Self) && !Self->isProhibited(to_select, card, targets);
}
...
const Card *XihuaCard::validate(CardUseStruct &card_use) const{
    ...
    if (user_string == "slash"
        ...
        if (!Config.BanPackages.contains("maneuvering"))
            nosguhuo_list << "normal_slash" << "thunder_slash" << "fire_slash";
        to_nosguhuo = room->askForChoice(yuji, "xihua_slash", nosguhuo_list.join("+"));
        yuji->tag["NosGuhuoSlash"] = QVariant(to_nosguhuo);
    }
    //room->broadcastSkillInvoke("xihua");
    ...
    if (xihua(card_use.from)) {
        //应当获取的是展示卡
		int id =yuji->tag["xihua_show"].toInt();
...
const Card *XihuaCard::validateInResponse(ServerPlayer *yuji) const{
    Room *room = yuji->getRoom();
    //room->broadcastSkillInvoke("nosguhuo");
...
public:
    ...
    virtual const Card *viewAs() const{
        ...
        if (c) {
            ...
            if (!c->objectName().contains("slash"))
                ...
                card->setUserString(Self->tag["XihuaSlash"].toString());
            //要填入展示的那张卡。。。
			//int id =Self->tag["xihua_show"].toInt();
			
			//const Card *x=Sanguosha->getCard(id);
			//card->addSubcard(x);
			//card->addSubcard(originalCard);
            return card;
        } else
    ...
    virtual QDialog *getDialog() const{
        return GuhuoDialog::getInstance("xihua");
    }

    virtual int getEffectIndex(const ServerPlayer *, const Card *card) const{
        if (!card->isKindOf("XihuaCard"))
            return -2;
        else
            return -1;
    }
    ...
    : Package("nostal_wind")
{
	...
	General *nos_dalizi = new General(this, "nos_dalizi", "qun", 3);
    nos_dalizi->addSkill(new Xihua);
