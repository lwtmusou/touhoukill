...
public:
    ...
    QList<SkillInvokeDetail> triggerable(TriggerEvent triggerEvent, const Room *room, const QVariant &data) const override
    {
        ...
        if (triggerEvent == EventPhaseChanging) {
            ...
            if ((kaguya1 != nullptr) && kaguya1->isAlive() && kaguya1->hasSkill(this) && move.from_places.contains(Player::PlaceHand) && kaguya1->getHandcardNum() != 4
                && kaguya1->getPhase() == Player::NotActive)
                kaguyas << kaguya1;
            if ((kaguya2 != nullptr) && kaguya2->isAlive() && kaguya2->hasSkill(this) && move.to_place == Player::PlaceHand && kaguya2->getHandcardNum() != 4
...
QList<SkillInvokeDetail> Ruizhi::triggerable(TriggerEvent /*triggerEvent*/, const Room * /*room*/, const QVariant &data) const
{
...
bool Ruizhi::cost(TriggerEvent /*triggerEvent*/, Room * /*room*/, QSharedPointer<SkillInvokeDetail> invoke, QVariant &data) const
{
    CardEffectStruct effect = data.value<CardEffectStruct>();
...
bool Ruizhi::effect(TriggerEvent /*triggerEvent*/, Room *room, QSharedPointer<SkillInvokeDetail> invoke, QVariant & /*data*/) const
{
    JudgeStruct judge;
...
public:
    ...
    bool effect(TriggerEvent /*triggerEvent*/, Room *room, QSharedPointer<SkillInvokeDetail> invoke, QVariant &data) const override
    {
        ...
        if (!targets.isEmpty()) {
            ...
            room->sendLog("#xushi_newTarget", use.from, use.card->objectName(), QList<ServerPlayer *>() << newTarget);
...
public:
    ...
    {
        DamageStruct damage = data.value<DamageStruct>();
        if ((damage.from != nullptr) && damage.from->isAlive() && damage.to->isAlive() && damage.to->hasSkill(this) && damage.from->getHandcardNum() > damage.to->getHandcardNum()
...
void MingmuCard::use(Room *room, const CardUseStruct &card_use) const
{
    ...
    while (!option.isEmpty() && mystia->isAlive()) {
        ...
        if (choice == "mingmu_give") {
            ...
            room->sendLog("#mingmuRange", source, "mingmu");
