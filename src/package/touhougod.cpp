...
public:
    ...
    static QList<ServerPlayer *> dongchaTargets(ServerPlayer *from, const Card *card)
    {
        QList<ServerPlayer *> targets;
        foreach(ServerPlayer *p, from->getRoom()->getOtherPlayers(from)) {
            if (from->inMyAttackRange(p)) {
                foreach(const Card *c, p->getCards("s")){
    ...
    QList<SkillInvokeDetail> triggerable(TriggerEvent e, const Room *room, const QVariant &data) const
    {
        ...
        if (e == CardUsed) {
            ...
            if (response.m_isUse && (response.m_card->isKindOf("BasicCard") || response.m_card->isNDTrick()) &&
                !dongchaTargets(response.m_from, response.m_card).isEmpty()) {
                foreach(ServerPlayer *p, room->findPlayersBySkillName(objectName())) {
                    if (response.m_from != p)
                        d << SkillInvokeDetail(this, p, p);
                }
            }
        }
    ...
    {
        QList<ServerPlayer *> targets;
        QString prompt = "@dongcha:";
        if (e == CardUsed) {
            CardUseStruct use = data.value<CardUseStruct>();
            targets = dongchaTargets(use.from, use.card);
            prompt = prompt +  use.from->objectName() + ":" + use.card->objectName();
        } else {
            CardResponseStruct response = data.value<CardResponseStruct>();
    ...
    bool effect(TriggerEvent e, Room *room, QSharedPointer<SkillInvokeDetail> invoke, QVariant &data) const
    {
        ...
        foreach(const Card *c, invoke->targets.first()->getCards("s")) {
            if (c->getTypeId() == card->getTypeId())
                disable << c->getEffectiveId();
        }


        int id = room->askForCardChosen(invoke->invoker, invoke->targets.first(), "s", objectName(), false, Card::MethodNone, disable);
        QList<int> ids;
        ids << id;
        invoke->targets.first()->removeShownHandCards(ids, true);
        if (e == CardUsed) {
            CardUseStruct use = data.value<CardUseStruct>();
...
public:
    ...
    void record(TriggerEvent triggerEvent, Room *room, QVariant &data) const
    {
        ...
        if (triggerEvent == CardUsed) {
            CardUseStruct use = data.value<CardUseStruct>();
            if (use.from->isCurrent() && use.card->isKindOf("Slash")) {
                if (use.from->hasFlag("tianqu_first"))
                    use.from->setFlags("tianqu_second");
                else
                    use.from->setFlags("tianqu_first");
            }
        }
    }
    ...
    QList<SkillInvokeDetail> triggerable(TriggerEvent triggerEvent, const Room *room, const QVariant &data) const
    {
        ...
        CardUseStruct use = data.value<CardUseStruct>();
        QList<SkillInvokeDetail> d;
        if (use.card->isKindOf("Slash") && use.from && use.from->isCurrent() && !use.from->hasFlag("tianqu_second")) {
    ...
    bool effect(TriggerEvent, Room *room, QSharedPointer<SkillInvokeDetail> invoke, QVariant &data) const
    {
        ...
        if (starter != NULL) {
            ...
            if (use.from && use.from->isAlive() && use.from != invoke->invoker)
                move = !room->askForUseSlashTo(invoke->invoker, use.from, "@tianqu");
            if (move) {
                starter->loseMark("@road");
                ServerPlayer *next = adjanceny(starter);
                next->gainMark("@road");
            }
        } else
...
class JifengVS : public ViewAsSkill
{
public:
    JifengVS() : ViewAsSkill("jifeng")
    {
        response_or_use = true;
    }
...
public:
    ...
    {
        events << CardUsed;
        view_as_skill = new JifengVS;
    }

    QList<SkillInvokeDetail> triggerable(TriggerEvent, const Room *room, const QVariant &data) const
    {
        CardUseStruct use = data.value<CardUseStruct>();
        if (use.card->getSkillName() == "jifeng" && use.card->isRed() && use.from->isAlive())
            return QList<SkillInvokeDetail>() << SkillInvokeDetail(this, use.from, use.from, NULL, true);
...
class JifengTargetMod : public TargetModSkill
{
public:
    JifengTargetMod() : TargetModSkill("#jifeng")
    {
        pattern = "Slash";
    }
    virtual int getDistanceLimit(const Player *from, const Card *card) const
    {
        if (card->getSkillName() == "jifeng")
            return 1000;
        return 0;
    }
};
    ...
    : Package("touhougod")
{
    ...
    aya_god->addSkill(new JifengTargetMod);
    related_skills.insertMulti("jifeng", "#jifeng");
