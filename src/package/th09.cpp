...
public:
    ...
    bool onPhaseChange(ServerPlayer * /*target*/) const override
    {
        return true;
    }
};
...
const Card *MianLingCard::validate(CardUseStruct & /*cardUse*/) const
{
...
const Card *MianLingCard::validateInResponse(ServerPlayer * /*user*/) const
{
    const Card *c = Sanguosha->getCard(subcards.first());
    c->setFlags("mianling");
    return c;
...
bool YucanSelectCard::targetsFeasible(const QList<const Player *> &targets, const Player * /*Self*/) const
{
...
bool YucanSelectCard::targetFilter(const QList<const Player *> & /*targets*/, const Player * /*to_select*/, const Player * /*Self*/) const
{
    Q_ASSERT(false);
    return false;
}

bool YucanSelectCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player * /*Self*/, int &maxVotes) const
{
...
void YucanSelectCard::onUse(Room * /*room*/, const CardUseStruct &card_use) const
{
    foreach (ServerPlayer *p, card_use.to)
...
bool HuiranCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player * /*Self*/) const
{
    return targets.length() < 2 && !to_select->isKongcheng();
}

void HuiranCard::onUse(Room * /*room*/, const CardUseStruct &card_use) const
{
    foreach (ServerPlayer *p, card_use.to)
...
{
    Room *room = effect.to->getRoom();
    room->askForDiscard(effect.to, getSkillName(), 1, 1, false, true);

    Analeptic *ana = new Analeptic(Card::NoSuit, 0);
    ana->setSkillName("_" + getSkillName());

    bool selfa = ana->isAvailable(effect.from);
    bool othera = ana->isAvailable(effect.to);
    ...
    bool selfchoice = selfa;

    if (selfa && othera)
        selfchoice = room->askForSkillInvoke(effect.from, getSkillName(), "drink::" + effect.to->objectName());

    CardUseStruct use;
    use.card = ana;
    use.from = (selfchoice ? effect.from : effect.to);
    room->useCard(use);
}

class Yuyan : public OneCardViewAsSkill
{
public:
    Yuyan()
        : OneCardViewAsSkill("yuyan")
    {
        filter_pattern = ".!";
    }
    ...
    bool isEnabledAtPlay(const Player *player) const override
    {
        return !player->hasUsed("YuyanCard");
    }

    const Card *viewAs(const Card *originalCard) const override
    {
        YuyanCard *c = new YuyanCard;
        c->addSubcard(originalCard);
        return c;
    }
};

class Zuimeng : public TriggerSkill
{
public:
    Zuimeng()
    ...
    QList<SkillInvokeDetail> triggerable(TriggerEvent /*triggerEvent*/, const Room *room, const QVariant &data) const override
    {
        QList<SkillInvokeDetail> d;
        PhaseChangeStruct change = data.value<PhaseChangeStruct>();
        if (change.to == Player::NotActive && !change.player->tag.value("touhou-extra", false).toBool() && !room->getThread()->hasExtraTurn()) {
            bool drank = false;
            foreach (ServerPlayer *p, room->getAllPlayers()) {
                if (p->getMark("drank") > 0 || p->getMark("magic_drank") > 0) {
                    drank = true;
                    break;
                }
            }
...
class Cuihua : public TriggerSkill
{
public:
    Cuihua()
        : TriggerSkill("cuihua$")
    ...
    {
        QList<SkillInvokeDetail> d;
        MarkChangeStruct m = data.value<MarkChangeStruct>();
        if ((m.name == "drank" || m.name == "magic_drank") && m.num < 0 && m.player->getMark(m.name) == 0 && m.player->getKingdom() == "zhan" && m.player->isAlive()) {
            foreach (ServerPlayer *p, room->getOtherPlayers(m.player)) {
                if (p->hasLordSkill(this) && p->isAlive() && p != m.player)
                    d << SkillInvokeDetail(this, p, m.player);
            }
        }
    ...
    : Package("th09")
{
    ...
    suika->addSkill(new Yuyan);
    suika->addSkill(new Zuimeng);
    suika->addSkill(new Cuihua);
