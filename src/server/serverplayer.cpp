...
QStringList ServerPlayer::checkTargetModSkillShow(const CardUseStruct &use)
{
    ...
    if (use.card->targetFixed(use.from) && !use.to.contains(use.from) && !use.card->isKindOf("AOE") && !use.card->isKindOf("GlobalEffect")) {
        if (isHiddenSkill("tianqu") && Sanguosha->currentRoomObject()->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_PLAY)
    ...
    //check prohibit
    foreach (ServerPlayer *p, use.to) {
        if (use.from->isProhibited(p, use.card) && Sanguosha->currentRoomObject()->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_PLAY) {
            showTargetProhibit << "tianqu";
            break;
        } else if (use.card->isKindOf("Peach")) {
            if (!p->isWounded() && Sanguosha->currentRoomObject()->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_PLAY) {
                showTargetProhibit << "tianqu";
                break;
            }
            if (p != use.from && (!p->hasLordSkill("yanhui") || p->getKingdom() != "zhan") && Sanguosha->currentRoomObject()->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_PLAY) {
                showTargetProhibit << "tianqu";
                break;
            }
        } else if (use.card->isKindOf("DelayedTrick") && p->containsTrick(use.card->objectName()) && Sanguosha->currentRoomObject()->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_PLAY) {
            showTargetProhibit << "tianqu";
            break;
        }
    }
