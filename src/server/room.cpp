...
QSharedPointer<TriggerDetail> Room::askForTriggerOrder(ServerPlayer *player, const QList<QSharedPointer<TriggerDetail> > &sameTiming, bool cancelable, const QVariant &)
{
    tryPause();
...
bool Room::makeSurrender(ServerPlayer *initiator)
{
    ...
    foreach (ServerPlayer *player, m_players) {
        ...
        if (player != initiator && player->isAlive() && player->getState() == "online") {
...
#endif // LOGNETWORK
        if (game_finished) {
            if (player && player->getState() == "online")
