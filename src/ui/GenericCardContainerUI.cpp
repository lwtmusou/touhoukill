...
void PlayerCardContainer::updateAvatar()
{
    ...
    if (m_player != nullptr) {
        ...
        if (ClientInstance->serverInfo()->isMultiGeneralEnabled() && getPlayer() == Self)
...
void PlayerCardContainer::updateSmallAvatar()
{
    ...
    if (general == nullptr && ClientInstance->serverInfo()->isMultiGeneralEnabled() && this->getPlayer() == Self) {
...
void PlayerCardContainer::updatePile(const QString &pile_name)
{
    ...
    QPoint start = (ClientInstance->serverInfo()->isMultiGeneralEnabled() && getPlayer() == Self) ? _m_layout->m_privatePileStartPosDouble : _m_layout->m_privatePileStartPos;
...
void PlayerCardContainer::refresh()
{
    if ((m_player == nullptr) || (m_player->general() == nullptr) || !m_player->isAlive()) {
        ...
        if ((_m_deathIcon != nullptr) && !(ClientInstance->serverInfo()->GameModeStr == QStringLiteral("04_1v3") && m_player->generalName() != QStringLiteral("yuyuko_1v32")))
...
void PlayerCardContainer::repaintAll()
{
    ...
    const char *face_turned_mask
        = isHegemonyGameMode(ClientInstance->serverInfo()->GameModeStr) ? QSanRoomSkin::S_SKIN_KEY_FACETURNEDMASK_HEGEMONY : QSanRoomSkin::S_SKIN_KEY_FACETURNEDMASK;
    ...
    if (!isHegemonyGameMode(ClientInstance->serverInfo()->GameModeStr) && _m_roleComboBox != nullptr)
        _m_roleComboBox->setPos(_m_layout->m_roleComboBoxPos);

    if (isHegemonyGameMode(ClientInstance->serverInfo()->GameModeStr) && _m_hegemonyroleComboBox != nullptr)
    ...
    QPoint magatamas_anchor = (ClientInstance->serverInfo()->isMultiGeneralEnabled() && getPlayer() == Self) ? _m_layout->m_magatamasAnchorDouble : _m_layout->m_magatamasAnchor;
...
void PlayerCardContainer::_createControls()
{
    ...
    if (ClientInstance->serverInfo()->isMultiGeneralEnabled() && this->getPlayer() == Self)
...
void PlayerCardContainer::killPlayer()
{
    ...
    refresh();
    if (ClientInstance->serverInfo()->GameModeStr == QStringLiteral("04_1v3") && !m_player->isLord()) {
        _m_deathIcon->hide();
        _m_votesGot = 6;
...
QVariant PlayerCardContainer::itemChange(GraphicsItemChange change, const QVariant &value)
{
    if (change == ItemSelectedHasChanged) {
        if (!value.toBool()) {
            ...
            QRect focusFrameArea
                = (this->getPlayer() == Self && ClientInstance->serverInfo()->isMultiGeneralEnabled()) ? _m_layout->m_focusFrameAreaDouble : _m_layout->m_focusFrameArea;
...
{
    int avatarSize = ((m_player->getGeneral2() != nullptr) || (ClientInstance->serverInfo()->isMultiGeneralEnabled() && this->getPlayer() == Self)) ? _m_layout->m_primaryAvatarSize
...
void PlayerCardContainer::showSkillName(const QString &skill_name, bool isSelf)
{
    if (!isSelf) {
        ...
        if (ClientInstance->serverInfo()->isMultiGeneralEnabled() && Self->inDeputySkills(skill_name))
...
QString PlayerCardContainer::getPlayerSkillDescription(Player *p, bool yellow, const QString &flag)
{
    ...
    foreach (const Skill *skill, skillList) {
        if (skill->isAttachedSkill())
            continue;
        if (!isHegemonyGameMode(ClientInstance->serverInfo()->GameModeStr) && !p->hasValidSkill(skill->name()))
            continue;
...
{
    QString basename;
    if (ClientInstance->serverInfo()->GameModeStr == QStringLiteral("06_3v3") || ClientInstance->serverInfo()->GameModeStr == QStringLiteral("06_XMode")) {
