...
void GuanxingBox::doGuanxing(const QList<int> &card_ids, bool up_only, QString skillName)
{
    ...
    foreach (int card_id, card_ids) {
        CardItem *card_item = new CardItem(ClientInstance->card(card_id));
        ...
        if (skillName == QStringLiteral("fengshui") && !up_items.isEmpty())
...
void GuanxingBox::adjust()
{
    ...
    if (skillName == QStringLiteral("fengshui") && !items->isEmpty())
