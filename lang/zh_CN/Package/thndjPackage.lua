...
--**********************************
	...
	["kexue"] = "科学",
	[":kexue"] = "若你处于连环状态，你于出牌阶段的空闲时间点使用的【杀】和普通锦囊牌无距离限制且额外目标数无上限。",
	["#kexue-effect"] = "科学",
...
--**********************************
	...
	["yaoli"] = "药理",
	[":yaoli"] = "“<font color=\"green\"><b>每名角色的出牌阶段限一次，</b></font>其可以弃置一张牌，若如此做，你可以摸一张牌，然后弃置一张牌，若弃置的这两张牌对应下列同一种分类，则当其于此阶段内使用下一张该类的牌指定目标时，触发相应效果：<br />基本：此牌的第一个效果值+1。<br />普通锦囊：其令一名角色成为此牌的额外目标或取消一个目标角色。<br />装备、延时类锦囊：其弃置一名角色的装备区和判定区里所有的牌。",
	["yaoliattach"] = "药理",
	[":yaoliattach"] = "<font color=\"green\"><b>你的出牌阶段限一次，</b></font>你可以弃置一张牌，若如此做，有“药理”技能的角色可以摸一张牌，然后弃置一张牌，若弃置的这两张牌对应下列同一种分类，则当你于此阶段内使用下一张该类的牌指定目标时，触发相应效果：<br />基本：此牌的第一个效果值+1。<br />普通锦囊：你令一名角色成为此牌的额外目标或取消一个目标角色。<br />装备、延时类锦囊：你弃置一名角色的装备区和判定区里所有的牌。",
	["yaolibasic"] = "药理（基本牌）",
