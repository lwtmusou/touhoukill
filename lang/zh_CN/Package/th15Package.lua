...
--***********************************
	...
	["chunhua"] = "纯化",
	[":chunhua"] = "当一名角色使用基本或普通锦囊牌指定角色为目标后，你可以选择一项：1.选择并暗置其一张明置手牌；2.（若使用的是明置手牌）选择此牌。然后根据选择的牌的颜色改变此牌效果：黑色，使用者对目标造成1点伤害；红色，此牌生效时，若为首个目标，则使用者和目标各摸一张牌，否则目标摸一张牌。<font color=\"green\"><b>每段限一次。</b></font>",
	...
	["$chunhua_removeshown_red"]   = "%from 发动了“纯化”选择了暗置 %to 的明置手牌 %card。将【%arg】的效果改为“使用者和所有目标各摸一张牌”",
	["$chunhua_selectuse_black"]   = "%from 发动了“纯化”选择了 %to 使用的明置手牌 %card。将【%arg】的效果改为“使用者对目标角色造成1点伤害”",
	["$chunhua_selectuse_red"]     = "%from 发动了“纯化”选择了 %to 使用的明置手牌 %card。将【%arg】的效果改为“使用者和所有目标各摸一张牌”",
